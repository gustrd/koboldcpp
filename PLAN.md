...
*   [ ] **Step 1.5c: Metal Backend Device Sync Path**
    ...
    *   **⚠️ Pitfalls:**
        1.  **`tensor->buffer` is NULL for disk-backed stubs.** Because `llama-model-loader.cpp:1511-1514` skips weight upload for `GGML_TENSOR_FLAG_DISK_BACKED` tensors, the tensor never gets a backend buffer assigned. Calling `ggml_backend_tensor_set()` on a tensor with `buffer == NULL` will hit the assert at `ggml-backend.cpp:293` (`buf != NULL && "tensor buffer not set"`). You must first allocate the tensor into a backend buffer (e.g. via `ggml_backend_alloc_ctx_tensors`) or assign a buffer manually before the first expert load.
        2.  **`ggml_backend_tensor_set` asserts `offset + size <= ggml_nbytes(tensor)`.** The `ggml_nbytes` calculation uses `ne[]` and `nb[]`. If the stride hack from `flash_moe_manager.cpp:113` makes `ggml_nbytes` return a different value than expected, this assert fires. Verify `ggml_nbytes` returns the correct total expert tensor size after the `nb[1]` override.
        3.  **Metal shared buffers require `newBufferWithBytesNoCopy` alignment.** When `ggml-metal` creates shared buffers with `newBufferWithBytesNoCopy`, the pointer must be `vm_page_size`-aligned (16KB on Apple Silicon). If Flash-MoE's allocation is only 4KB-aligned, Metal will reject the buffer creation. The `posix_memalign` in `flash_moe_cache.cpp:22` uses 4096 — this must be increased to `vm_page_size` on macOS.
        4.  **`prepare_nodes` lacks backend handle.** It's called at `ggml-backend.cpp:1593` inside `ggml_backend_sched_compute_splits` but receives only `(ggml_tensor**, int)`. To call `ggml_backend_tensor_set`, you need to know which backend owns the tensor. Thread the `split->backend` through to the prepare function, or read it from `tensor->buffer->buft->device`.
...
*   [ ] **Step 2.1: Unify ExpertManager and SlotBufferAllocator**
    ...
    *   **⚠️ Pitfalls:**
        1.  **The LRU cache uses fixed-size slots (`bytes_per_slot`) but experts vary in size per projection.** `flash_moe_manager.cpp:69-71` shows gate/up/down projections have different byte sizes (`gate_bytes`, `up_bytes`, `down_bytes`). The `SlotBufferAllocator` allocates uniform slots. Either: (a) make slot size = full expert size (gate+up+down combined), loading the whole expert file per slot, or (b) have three separate caches per projection type. Option (a) is simpler and matches the file layout (`blk00_exp000.bin`).
        2.  **Double-caching hazard.** If the manager commits virtual pages AND the LRU cache holds copies in its own pool, you have two copies of the same expert in RAM. Decide: either the LRU pool IS the committed virtual memory (use `mmap` to map the pool into the reservation range), or abandon the virtual reservation entirely and just use the LRU pool directly. The latter is simpler.
        3.  **`get_expert_sync` holds `cache_mutex` during I/O.** At `flash_moe_cache.cpp:52`, the lock guard holds the mutex for the entire function including the `read_direct_io` call. A 1ms SSD read means the mutex is held for 1ms, blocking all other expert lookups. For the synchronous MVP this is acceptable but must be noted for Phase 3.
